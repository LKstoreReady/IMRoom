...
class HeGetMousePosition: NSObject {
    ...
    init(_ dic : NSDictionary) {
        ...
        aScreenW = String(format: "%d", dic.wxLaunchId(pSharpQuickreply))
        jImageTransformer = dic.scannerWithString(jTryAppend)
        oWeakObserver = String(format: "%d", dic.wxLaunchId(dForceLoad))
        vHeroLogo = String(format: "%d", dic.wxLaunchId(wFeatureMap))
        ...
        bConditionalModifiers = dic.setTimeInSeconds(hLogImpl)
    ...
    class func performBlockAndWait(_ array : NSArray) -> NSArray {
        ...
        for iDispatchCancel in 0..<array.count {
            ...
            if vTokenizeString != nil {
                let oBundledPath = HeGetMousePosition(vTokenizeString!)
